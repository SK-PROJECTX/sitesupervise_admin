...
export default function ConferencePage() {
  ...
  if (view === "schedule") {
    return (
      <div className="min-h-screen bg-[#021422] flex flex-col items-center justify-center p-6 md:p-8">
        <div className="max-w-4xl w-full space-y-8 md:space-y-12">
          <h1 className="text-3xl md:text-4xl font-bold text-white mb-8 md:mb-12 text-center md:text-left">
            Meeting Schedule
          </h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            ...
            >
              <Video className="w-12 h-12 md:w-16 md:h-16 text-[#021422] group-hover:text-[#0070D4] transition-colors" />
              <span className="font-bold text-[#021422] text-base md:text-lg">
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-200 relative overflow-hidden">
      {/* Header */}
      <div className="bg-white px-4 md:px-6 py-3 md:py-4 flex flex-col sm:flex-row items-center justify-between shadow-sm z-20 gap-2 sm:gap-0">
        <div className="flex items-center gap-3 md:gap-4">
          <Building2 size={24} className="text-[#021422] md:w-8 md:h-8" />
          <h1 className="text-lg md:text-xl font-bold text-[#021422]">
            Project Room
          </h1>
        </div>
        <div className="text-gray-500 font-mono text-sm md:text-xl">
          Meeting Time: {meetingTime}
        </div>
      </div>

      {/* Main Content */}
      <div className="flex-1 flex flex-col lg:flex-row p-4 md:p-6 gap-4 md:gap-6 overflow-hidden relative">
        ...
        <AnimatePresence>
          {activeOverlay === "controls" && (
            ...
            >
              <div className="bg-[#021422] rounded-3xl p-6 md:p-8 w-full max-w-[500px] pointer-events-auto relative shadow-2xl">
          ...
          {activeOverlay === "chat" && (
            <motion.div
              ...
              animate={{ x: 0 }}
              exit={{ x: "100%" }}
              className="absolute right-0 top-0 bottom-0 w-full md:w-[400px] bg-white z-40 shadow-2xl flex flex-col"
            >
          ...
          {activeOverlay === "agenda" && (
            <motion.div
              ...
              className="absolute left-1/2 -translate-x-1/2 top-24 lg:left-[360px] lg:translate-x-0 z-40 p-4 w-full max-w-[350px]"
            >
              <div className="bg-white rounded-xl shadow-2xl w-full overflow-hidden">
                ...
                <div className="p-6">
                  <div className="flex items-center gap-2 mb-4">
                    <span className="text-[#0070D4] font-bold text-sm">
                      Status:
                    </span>
                  ...
                  <div>
                    <span className="text-[#0070D4] font-bold text-sm block mb-2">
                      Latest:
                    </span>
          ...
          {activeOverlay === "actions" && (
            <motion.div
              ...
              className="absolute inset-0 z-50 flex items-center justify-center pointer-events-none p-4"
            >
              <div className="bg-white rounded-3xl w-full max-w-[600px] pointer-events-auto relative shadow-2xl overflow-hidden">
                <div className="bg-[#021422] p-6 flex justify-between items-center text-white">
                ...
                <div className="p-12 flex flex-col items-center text-center gap-6">
                  <div className="w-16 h-16 bg-green-500 rounded-lg flex items-center justify-center shadow-lg shadow-green-200">
                    <Plus className="text-white stroke-[4]" size={32} />
                  </div>
                  <div className="space-y-4">
                    <h2 className="text-2xl md:text-3xl font-bold text-[#021422] leading-tight">
                      Action Created: Verify rebar spacing at NE Corner
                    </h2>
                    <p className="text-gray-500 font-bold text-sm md:text-base tracking-wide">
                      Owner: John Smith | Due: EOD | Linked: WP-205
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="w-full lg:w-80 flex flex-col gap-4 z-0">
          {/* Floating Actions */}
          <div className="flex flex-row lg:flex-col gap-2 flex-wrap">
            <button
              onClick={() =>
                setActiveOverlay(activeOverlay === "agenda" ? null : "agenda")
              }
              className="bg-[#021422] text-white px-3 md:px-4 py-3 md:py-4 rounded-lg border border-gray-600 text-[10px] md:text-xs font-bold hover:bg-gray-800 flex items-center gap-2 w-fit"
            >
              <FileText size={14} /> AGENDA ITEM
            </button>
            <button
              onClick={() =>
                setActiveOverlay(activeOverlay === "actions" ? null : "actions")
              }
              className="bg-[#0070D4] text-white px-3 md:px-4 py-3 md:py-4 rounded-lg text-[10px] md:text-xs font-bold hover:bg-[#005bb5] flex items-center gap-2 w-fit"
            >
          ...
          <div className="bg-[#021422] rounded-xl">
            ...
            <div className="flex lg:flex-col gap-3 md:gap-4 px-3 pt-3 overflow-x-auto lg:overflow-x-visible pb-3 lg:pb-0 scrollbar-hide">
              {participants.map((p, i) => (
                <div
                  key={i}
                  className="relative aspect-video bg-gray-900 rounded-xl overflow-hidden group shadow-lg flex-shrink-0 w-48 md:w-64 lg:w-full"
                >
                  <img
                    src={p.image}
                    alt={p.name}
        ...
        <div className=" flex-1 flex flex-col gap-6">
          <div className="flex flex-row lg:flex-col gap-2 justify-start lg:justify-end items-center lg:items-end flex-wrap">
            <button
            ...
            <button
              onClick={() =>
                setActiveOverlay(
                  activeOverlay === "controls" ? null : "controls",
                )
              }
              className="bg-[#0070D4] text-white px-3 md:px-4 py-3 md:py-4 rounded-lg text-[10px] md:text-xs font-bold hover:bg-[#005bb5] flex items-center gap-2 w-fit"
            >
          ...
          <div className="flex-1 bg-[#021422] rounded-2xl flex flex-col overflow-hidden relative shadow-2xl">
            <div className="p-8 pb-0">
              <h2 className="text-white font-bold text-lg tracking-wider text-center uppercase mb-8">
                SHARED WORKSPACE
              </h2>
              ...
              <div className="flex justify-center gap-4 mb-8">
                ...
                <button className="px-4 md:px-6 py-2 bg-gray-100 text-[#021422] rounded-lg flex items-center gap-2 text-xs md:text-sm font-bold">
                  <Activity size={14} className="md:w-4 md:h-4" /> Live Site
                  Feed
                </button>
              </div>
            </div>

            <div className="flex-1 bg-gray-200 mx-4 md:mx-8 mb-4 md:mb-8 rounded-2xl relative flex items-center justify-center border-2 md:border-4 border-white/10 overflow-hidden">
              ...
              <div className="text-center">
                ...
                <p className="text-gray-400 text-[10px] mt-1">
                  Block A, 5th Floor (John's View)
                </p>
              </div>

              <div className="absolute bottom-2 left-0 right-0 text-center">
                <p className="text-[10px] text-[#021422] font-semibold">
                  Live Video Showing Rebar. Remote Markups Visible.
                </p>
              </div>
            </div>
          </div>
